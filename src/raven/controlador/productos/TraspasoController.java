...
public class TraspasoController {
    ...
    public TraspasoController(int idUsuario) {
        ...
        this.idUsuarioActual = (sid > 0) ? sid : idUsuario;
        System.out.println("TraspasoController inicializado con usuario ID: " + this.idUsuarioActual);
    }
    ...
    public List<Bodega> cargarBodegas() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                // CORRECCIÓN: Verificar si capacidad_maxima puede ser null
                Integer capacidadMaxima = rs.getObject("capacidad_maxima") != null ? rs.getInt("capacidad_maxima")
                        : null;
                bodega.setCapacidadMaxima(capacidadMaxima);
                bodega.setActiva(rs.getBoolean("activa"));

                bodegas.add(bodega);
            }

            System.out.println("Bodegas cargadas: " + bodegas.size());
    ...
    public StockVerificationResult verificarStockDisponible(Integer idProducto, Integer idVariante, String tipo,
        ...
        try {
            ...
            if (!stockSuficiente) {
                int stockActual = obtenerStockActualEnBodega(idProducto, idVariante, tipo,
                        traspasoActual.getIdBodegaOrigen());
    ...
    // producto_variantes
    private int obtenerStockActualEnBodega(Integer idProducto, Integer idVariante, String tipo, Integer idBodega) {
        String columna = "caja".equalsIgnoreCase(tipo) ? "COALESCE(ib.Stock_caja,0)" : "COALESCE(ib.Stock_par,0)";
        String sql;
        if (idVariante != null) {
            sql = "SELECT " + columna + " AS stock "
        ...
        try {
            ...
            e.printStackTrace();
            System.err.println("ERROR  Error obteniendo stock: " + e.getMessage());

            // FALLBACK: sumar sin bodega específica
            return obtenerStockFallback(idProducto, idVariante, tipo);
        } finally {
            try {
                if (rs != null) {
                    rs.close();
    ...
    // MÉTODO NUEVO - Fallback para obtener stock desde producto_variantes
    private int obtenerStockFallback(Integer idProducto, Integer idVariante, String tipo) {
        String columna = "caja".equalsIgnoreCase(tipo) ? "COALESCE(ib.Stock_caja,0)" : "COALESCE(ib.Stock_par,0)";
        String sql;
        if (idVariante != null) {
            sql = "SELECT COALESCE(SUM(" + columna + "),0) AS stock "
