...
public class TableCellProfile extends javax.swing.JPanel {
    ...
    public TableCellProfile(ModelProduct data, Font font) {// Ruta de la imagen
             
         initComponents();
        lbName.setFont(font);
        lbLocation.setFont(font);
        lbName.setText(data.getName());
        lbLocation.setText(data.getGender());
        lbLocation.putClientProperty(FlatClientProperties.STYLE, 
            "foreground:$Label.disabledForeground");
        
        // 1. Obtener el perfil de manera segura
        ModelProfile profile = data.getProfile();
        
        // 2. Si el perfil es null, usar uno por defecto
        if (profile == null) {
            profile = new ModelProfile(); // Perfil con valores por defecto
        }
        
        // 3. Obtener el avatar (maneja automáticamente iconos null)
        pic.setIcon(profile.getAvatarIcon());
