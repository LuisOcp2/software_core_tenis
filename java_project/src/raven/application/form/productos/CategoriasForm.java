...
import raven.application.form.productos.creates.CreateCategory;
...
public class CategoriasForm extends javax.swing.JPanel {
    ...
    private final ServiceCategory service = new ServiceCategory();
    ...
    private void loadData() throws SQLException {
        try {
            ...
            List<ModelCategory> list = service.getAll();

            // Añade cada empleado como fila en la tabla
            for (ModelCategory d : list) {
    ...
    private void searchData(String search) throws SQLException {
        ...
        List<ModelCategory> list = service.search(search);
    ...
    private void initComponents() {
        ...
        lb.setText("GESTION DE CATEGORIAS");
    ...
    private void btn_nuevoActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn_nuevoActionPerformed
        CreateCategory cr = new CreateCategory();
        ...
        SimpleModalBorder border = new SimpleModalBorder(cr, "Nuevo Producto", options, (ModalController mc, int i) -> {
            if (i == SimpleModalBorder.OK_OPTION) {
                ...
                runModalActionWithButtonLoading(
                        ...
                        () -> {
                            Toast.show(CategoriasForm.this, Toast.Type.SUCCESS, "Categoría creada exitosamente");
                            try {
                                loadData();
                            } catch (SQLException ex) {
                                Toast.show(CategoriasForm.this, Toast.Type.ERROR,
                        ...
                        (Exception ex) -> {
                            ...
                            Toast.show(CategoriasForm.this, Toast.Type.ERROR, "Error: " + cause.getMessage());
                        });
    ...
    private void btn_editarActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btn_editarActionPerformed
        List<ModelCategory> list = getSelectedData();

        if (!list.isEmpty()) {
            if (list.size() == 1) {
                ModelCategory data = list.get(0);
                CreateCategory createc = new CreateCategory();
                                ...
                                (mc, i) -> {
                                    if (i == SimpleModalBorder.OK_OPTION) {
                                        ...
                                        runModalActionWithButtonLoading(
                                                ...
                                                () -> {
                                                    try {
                                                        ModelCategory dataEdit = createc.getData();
                                                        dataEdit.setCategoryId(data.getCategoryId());
                                                ...
                                                () -> {
                                                    Toast.show(this, Toast.Type.SUCCESS, "Categoría actualizada");
                                                    try {
                                                        loadData();
                                                    } catch (SQLException ex) {
                                ...
                                if (i == SimpleModalBorder.OK_OPTION) {
                                    ...
                                    runModalActionWithButtonLoading(
                                            ...
                                            () -> {
                                                try {
                                                    for (ModelCategory d : list) {
                                                        service.delete(d.getCategoryId());
                                                    }
                                                } catch (SQLException ex) {
                                                    throw new RuntimeException(ex);
                                                }
                                            ...
                                            () -> {
                                                Toast.show(this, Toast.Type.SUCCESS, "Categoría(s) eliminada(s)");
                                                try {
                                                    loadData();
                                                } catch (SQLException ex) {
                                                    Toast.show(this, Toast.Type.ERROR,
    ...
    private List<ModelCategory> getSelectedData() {
        ...
        for (int i = 0; i < table.getRowCount(); i++) {
            ...
            if ((boolean) table.getValueAt(i, 0)) {
                ...
                // actual
                ModelCategory data = (ModelCategory) table.getValueAt(i, 2);
                list.add(data);
            }
        }
