            ...
            protected void done() {
                try {
                    ...
                    if (bodegas != null && !bodegas.isEmpty()) {
                        ...
                        // SEGURIDAD: Restringir bodega origen para usuarios no administradores
                        if (!SessionManager.getInstance().isCurrentUserAdmin()) {
                            Integer userBodegaId = SessionManager.getInstance().getCurrentUserBodegaId();
                            if (userBodegaId != null) {
                                for (int i = 0; i < cbxOrigen.getItemCount(); i++) {
                                    Object item = cbxOrigen.getItemAt(i);
                                    if (item instanceof Bodega && ((Bodega) item).getIdBodega().equals(userBodegaId)) {
                                        cbxOrigen.setSelectedIndex(i);
                                        cbxOrigen.setEnabled(false);
                                        System.out.println("   SECURITY Bodega origen restringida a: " + ((Bodega) item).getNombre());
                                        break;
                                    }
                                }
                                
                                // Asegurarse de que el controlador tenga la bodega correcta asignada
                                if (controller != null && controller.getTraspasoActual() != null) {
                                    controller.getTraspasoActual().setIdBodegaOrigen(userBodegaId);
                                }
                            }
                        }
