...
public class traspasos extends javax.swing.JPanel {
    ...
    public traspasos() {
        // ═══════════════════════════════════════════════════════════════════════════
        // INICIALIZACIÓN CRÍTICA: Servicios y Controladores antes de configurar la UI
        // ═══════════════════════════════════════════════════════════════════════════
        this.traspasoService = new TraspasoService();
        try {
            // Sincronizar con nueva arquitectura de sesión
            UserSession userSession = UserSession.getInstance();
            
            // Compatibilidad: Si SessionManager tiene al usuario pero UserSession no, sincronizar
            if (userSession.getCurrentUser() == null && SessionManager.getInstance() != null) {
                ModelUser oldUser = SessionManager.getInstance().getCurrentUser();
                if (oldUser != null) {
                    userSession.setCurrentUser(oldUser);
                }
            }
            ...
            if (userSession.getCurrentUser() != null) {
                this.idUsuarioActual = userSession.getCurrentUser().getIdUsuario();
            } else {
                this.idUsuarioActual = 1;
            }

        } catch (Exception e) {
            System.err.println("CRITICAL: Error inicializando TraspasoController: " + e.getMessage());
            this.idUsuarioActual = 1;
            this.controller = new TraspasoController(UserSession.getInstance());
        }
        ...
        new SwingWorker<List<Bodega>, Void>() {
            ...
            protected void done() {
                try {
                    ...
                    // Seleccionar bodegas por defecto y aplicar restricciones según ROL
                    raven.clases.admin.UserSession us = raven.clases.admin.UserSession.getInstance();
                    boolean isAdmin = us.hasRole("admin");
                    Integer idBodegaUser = us.getIdBodegaUsuario();
                    
                    Integer defOrigen = us.getDefaultBodegaOrigen();
                    Integer defDestino = us.getDefaultBodegaDestino();
                    
                    if (defOrigen != null) seleccionarBodegaEnCombo(combo1, defOrigen);
                    if (defDestino != null) seleccionarBodegaEnCombo(combo3, defDestino);
                    
                    // Lógica de restricciones para NO-ADMIN
                    if (!isAdmin && idBodegaUser != null && idBodegaUser > 0) {
                        if ("ENVIADO".equals(filtroDireccion)) {
                            // Si es enviado, el origen debe ser su bodega
                            seleccionarBodegaEnCombo(combo1, idBodegaUser);
                            combo1.setEnabled(false);
                            combo3.setEnabled(true);
                        } else {
                            // Si es recibido, el destino debe ser su bodega
                            seleccionarBodegaEnCombo(combo3, idBodegaUser);
                            combo3.setEnabled(false);
                            combo1.setEnabled(true);
                        }
                    } else {
                        // Admins tienen libertad total
                        combo1.setEnabled(true);
                        combo3.setEnabled(true);
                    }
    ...
    private List<Object[]> obtenerTraspasos(String estado, Object bodegaOrigen, Object bodegaDestino, String fecha) {
        ...
        try {
            ...
            // ═══════════════════════════════════════════════════════════════════════════
            // FILTRO AUTOMÁTICO POR BODEGA DEL USUARIO Y DIRECCIÓN (Solo si no es ADMIN)
            // ═══════════════════════════════════════════════════════════════════════════
            boolean userIsAdmin = raven.clases.admin.UserSession.getInstance().hasRole("admin");
            if (!userIsAdmin && idBodegaUsuario != null && idBodegaUsuario > 0) {
                if ("ENVIADO".equals(filtroDireccion)) {
