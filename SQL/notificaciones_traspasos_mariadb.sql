...
-- Eliminar índice único heredado que genera conflictos por llamadas duplicadas
DROP INDEX IF EXISTS idx_notif_traspaso_evento ON notificaciones;
...
BEGIN
  ...
  WHERE u.activo = 1 AND u.id_bodega = v_target
    AND NOT EXISTS (
      ...
      WHERE n.tipo_referencia = 'traspasos'
        AND n.id_referencia = p_id_traspaso
        AND n.evento = p_evento
        AND n.activa = 1
    );
END $$
DELIMITER ;
